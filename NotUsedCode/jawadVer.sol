...
contract DHUCoin is StandardToken{
    ...
    event StudentInfo(address studentAddress, string studentID, string firstName, string lastName, uint gpa, string usagePeriod, uint usageCount);
    ...
    modifier miningAllowed{
        ...
        require(keccak256(student.usagePeriod) == keccak256(periodName));
        require(student.usageCount == 0);
    ...
    uint256 startBlockInput = 2873300;
    uint256 endBlockInput = 2873600;
    ...
    function changePerod(string _periodName) external onlyMainWallet{
        periodName = _periodName;
        for(uint i = 0; i < studentsAccounts.length; i ++){
            Student storage curStudent = students[studentsAccounts[i]];
            curStudent.usagePeriod = _periodName;
            curStudent.usageCount = 0;
        }
    }
    ...
    function addStudent(string _studentID, string _firstName, string _lastName, uint _gpa) public{
        ...
        var oldUsagePeriod = "";  
        uint oldUsageCount = 0;
        bool studentAlreadyPresent;
        ...
        for(uint i = 0; i < studentsAccounts.length; i ++){
            if(msg.sender == studentsAccounts[i]){
                ...
                oldUsagePeriod = students[msg.sender].usagePeriod;
                oldUsageCount = students[msg.sender].usageCount;
        ...
        if(studentAlreadyPresent){
            student.usagePeriod = oldUsagePeriod;
            student.usageCount = oldUsageCount;
        }else{
            student.usagePeriod = periodName;
        ...
        emit StudentInfo(msg.sender, _studentID, _firstName, _lastName, _gpa, student.usagePeriod, student.usageCount);
    }
    ...
    function getOneStudent(address _address) view public returns (string, string, string, uint, string, uint) {
        Student storage student = students[_address];
        return (student.studentID, student.firstName, student.lastName, student.gpa, student.usagePeriod, student.usageCount);
    }
    ...
    function proofOfWork(uint256 privKey) miningAllowed public {
        ...
        students[msg.sender].usageCount = 1;
